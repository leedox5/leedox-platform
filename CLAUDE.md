...
- **DEV→HQ 콘텐츠 제안 및 양방향 정제 채널 공식화 (HQ AGENTS.md 제6항 / 2026-09-01):**
  - 신규 기획 및 초안 작성은 HQ가 주도하되, **DEV 에이전트는 플랫폼 뷰어 UX 및 마크다운 렌더링 품질 향상을 위한 콘텐츠 정제/리팩토링 권한**을 갖는다.
  - DEV가 뷰어 최적화(제목 정제, 중복/불필요 정적 링크 제거, Heading 렌더링 조정 등)를 수행한 경우, HQ 저장소로 역SYNC 알림을 보내고 HQ 에이전트가 검증 후 HQ Git에 커밋/푸시하여 양방향 콘텐츠 정합성을 완벽히 유지한다.
...
- **Railway 환경변수(`railway variables set` 또는 대시보드)를 바꾸면 서비스가 자동 재시작된다** — 별도로 재배포나 재시작을 트리거할 필요가 없다(2026-08-24, `PAYMENT_PROVIDER`/`PORTONE_KAKAOPAY_CHANNEL_KEY` 설정 직후 `railway logs`에 새 부팅 로그가 바로 찍히는 것으로 확인). 다만 코드가 그 변수를 아직 안 읽는 상태(옛 배포)에서 변수부터 넣으면, 재시작된 옛 코드가 그 변수 조합으로 즉시 동작하기 시작한다 — 변수와 그걸 쓰는 코드는 항상 같이 배포 순서를 맞출 것(먼저 코드 push, 배포 확인 후 변수 설정).

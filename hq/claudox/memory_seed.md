...
13. **HQ/DEV 단축어** (2026-07-14부터) — "여기"/"거기" 대신 `HQ`=leedox-hq(이 저장소, 2026-08-03 이전 이름 chatdox-curriculum), `DEV`=leedox-platform(실제 코드 구현 쪽, 2026-08-03 이전 이름 chatdox-platform)로 부른다. `claudox/97_commands.md` 표에도 등록됨.
15. **DEV는 이 PC에 체크아웃이 두 곳, 역할이 다름** (2026-07-14 제정, 2026-07-16 정정, 2026-08-03 저장소명 갱신) — WSL 네이티브 클론(`~/dev/leedox-platform`)이 **진짜 DEV**(Platform Agent가 실제로 개발·커밋하는 곳). Windows 마운트 클론(`D:\RubyOnRails\leedox-platform`)은 **HQ의 read-only 참조용 체크아웃** — 코드 읽기 전용, 여기서 commit/push 하면 안 됨(2026-07-16 실수로 한 번 어겨서 Tommy가 DEV에 직접 정정함, 커밋 e43e689). Windows 쪽은 CRLF 때문에 `bin/rails` 셔뱅도 깨짐.
...
24. **새 Repo·새 팀으로 크게 벌이는 시도는 이 프로젝트의 축적된 맥락을 안 갖고 나간다** (2026-07-29) — Tommy가 "Chatdox 시즌(S01/S02)" 확장을 별도 Repo·별도 에이전트 팀으로 10라운드에 걸쳐 시도했다가 폐기(`.local/handoff/0001/_VERDICT.md`, `.local/dev/REPORT-01_chatdox_s02_review.md` 참고). 원인은 "조사"로 시작한 게 스스로 제안한 로드맵을 그대로 승인받아 커졌고, 이 프로젝트가 이미 겪은 교훈(23번 항목의 `railway ssh` 등)이 새 환경에 이관 안 돼 같은 실수가 반복됐고, 마지막 프로덕션 승인마저 실제 화면을 직접 안 열어보고 자동화 리포트만으로 통과시킨 것. 실제 코드는 chatdox-platform main에 병합·배포까지 됐다가 `leedox_revert_chatdox_season_experiment_r1`로 되돌림. 앞으로 이런 제안이 나오면 "이 흐름 밖에서 하면 지금까지 쌓은 규율·교훈이 자동으로 안 넘어간다"는 걸 먼저 짚는다. 일반화한 서사는 `claudox/17_workflow_scale.md`("규모를 키우려다 놓친 것") 반영. **2026-08-08 재확인**: Cowork 모드에서 handoff 0027을 완주한 뒤 "Claudox Season 2"로 확장하고 싶은 유혹이 실제로 다시 왔는데, 이번엔 착수 전에 이 항목(과 `_VERDICT.md`)을 먼저 찾아보고 접었다 — `claudox/93_cowork_first_handoff.md`에 결말로 반영. 이 기록 자체가 두 번째 유혹을 막아낸 첫 사례.
25. **제품 백로그(`backlog/`, repo root) 신설** (2026-08-08) — `service-desk/backlog.md`(실행 단위 대기열)와 별개로, "LEEDOX가 무엇을 만들지"에 대한 제품/UX/콘텐츠 아이디어를 쌓는 새 폴더. 항목마다 파일 하나(`backlog/NNNN_slug.md`, `Idea → Ready → Promoted/Dropped` 상태), `INDEX.md`로 전체 현황 스캔, 규칙은 `backlog/GUIDE.md`. 번호는 항상 "backlog NNNN"으로 불러 handoff/service-desk/챕터 번호와 구분한다. 다음 handoff를 고를 땐 여기부터 검토. `CLAUDE.md`에도 섹션 등록. 이 재설계 과정 자체가 handoff를 git 기반으로 바꿨던 사례와 같은 패턴이라 `claudox/17_workflow_scale.md`("이번엔 백로그였다")에 반영.
26. **Cowork 격리는 결함이 아니라 기존 원칙의 강제 적용** (2026-08-08) — Cowork 샌드박스에서 git push가 프록시에서 막히는 이유를 조사하다가(공식 아키텍처 문서 + GitHub 이슈), VSCode 확장(로컬 프로세스, 실제 자격증명·네트워크 그대로 사용)과 Cowork(파일 도구만 권한 체크 후 브릿지, 셸/코드 실행은 완전히 격리된 VM)의 구조 차이를 확인했다. 이 격리가 우연히도 이 프로젝트가 이미 14번 규칙으로 적어둔 원칙("HQ는 DEV 저장소에 직접 commit/push 안 함, 항상 handoff 채널로만")과 같은 방향이라는 게 핵심 발견 — Cowork로 팀 구성을 통째로 바꾸자는 게 아니라, DEV는 Claude Code(정밀 코드 작업에 적합), HQ는 Cowork여도 무방(관찰·조율이 본업이라 격리가 오히려 기존 경계를 재현)하다는 역할별 정리로 귀결. 코드 접근 없이 쓴 handoff 0027 요청서도 "추정 경로, Platform 재확인" 식으로 방어적으로 썼는데, 이게 오히려 `leedox_home_r1/guide.md`의 "완성 코드를 미리 쓰지 않는다" 원칙을 강제로 지키게 했다. `claudox/94_box_reflected_old_principles.md`(93장 자매편)에 반영.

27. **Cowork 세션의 push는 매 커밋마다 필요하지 않다** (2026-08-08) — Cowork 샌드박스는 `git push`가 항상 403(프록시 차단)이라 그동안 커밋할 때마다 Tommy에게 수동 push를 요청해왔는데, Tommy가 "DEV는 HQ의 push 없이도 handoff 내용을 전달받으니 매번 요청 안 해도 된다"고 정정 — Platform Agent에게 전달되는 경로가 GitHub push 자체가 아니기 때문. 이제부터는 Cowork 세션에서 커밋은 계속 매 단계마다 하되, push는 세션이 끝날 때 한 번(또는 Tommy가 명시적으로 `SYNC`/push를 요청할 때)만 하면 된다 — 매 handoff 발행·Confirmed마다 "수동 push 부탁드려요"라고 묻지 않는다.
28. **handoff는 여러 라운드(R1, R2...)로 이어갈 수 있다** (2026-08-08, backlog 0011→handoff 0032에서 첫 적용) — 조사(R1)로 끝난 handoff의 판단이 나중에 뒤집히면, 새 handoff 번호를 만들지 않고 같은 패키지 안에서 라운드를 이어간다. 요청서(`0000.md`)에 `## R2. 요청 사항` 섹션을 이어붙이고, DEV 결과 파일은 R2부터 `result_r2.md`로 라운드 번호를 붙이되 R1의 `result.md`는 리네임하지 않으며, 최종 확인 문서(`STATUS.md`)는 handoff 전체가 끝날 때 한 번만 쓴다. 규칙 전문은 `handoff/GUIDE.md`, 서사는 `claudox/17_workflow_scale.md`("이번엔 handoff 라운드였다")에 반영.
29. **Cowork 첫 세션 마무리 (2026-08-08)** — handoff 0027~0034(0032는 R1+R2) 전부 Confirmed로 종료, `backlog/`·`handoff/GUIDE.md` 신설, 게스트 시점 사이트 전수 점검(0010/0011/0012 발견 → 전부 해결, 후속으로 0015/0016/0017 등록)까지 마친 첫 Cowork 세션. 세션 마지막에 Platform Agent가 요청 없이 회고 문서(`handoff/0034_.../result_retrospection.md`)를 보내 자기 실수(CI 환경 미반영 2회)를 스스로 밝혔고, 이걸 읽고 나서야 HQ 쪽 사각지대(그날 내내 CI 상태를 한 번도 확인 안 함)도 드러났다 — `claudox/16_subagent.md`(서브에이전트·독립 에이전트가 실제로 보여준 것)와 `claudox/94_box_reflected_old_principles.md`("CI도 보지 않았다는 걸 알았다")에 반영. 이 세션의 전체 맥락은 `claudox/93·94장`이 담고 있다.

30. **표시 계층 handoff는 뷰어 상태별 기대 결과를 요청서에 다 적는다** (2026-08-11, handoff 0044에서 제정) — 페이지가 *무엇을 보여주는지*를 바꾸는 요청(배지, 잠금 아이콘, 개수, 이용 안내 문구)은 `0000.md`에 게스트가 볼 화면과 라이선스 보유자가 볼 화면을 각각 적고, DEV에게 두 상태 다 확인해 보고하라고 요청한다. HQ는 비로그인 화면만 검증할 수 있으므로(대신 로그인하지 않는다 — `claudox/98_hands_off_credentials.md`), 로그인 상태에서만 참인 것은 미리 적어두지 않으면 아무도 안 본 채로 넘어간다. 그리고 표시 조건은 **실제 게이트가 쓰는 접근 권한 판정을 그대로 재사용**하고 표시 전용 조건을 새로 만들지 않는다 — 그 분리가 정확히 backlog 0019가 겪은 문제다. 계기: handoff 0044 R1 요청서에 "`🔒 잠금` 표시를 붙인다"고 무조건으로 써서 라이선스 보유자에겐 사실과 다른 화면이 될 뻔했고, Tommy가 검토 중 "라이선스가 있는 유저라면 맞지 않을 수도 있는데"라고 잡아내 R2-2로 정정했다.

31. **전제는 요청서에 적기 전에, 그리고 판단 근거로 쓰기 전에 검증한다** (2026-08-16, 하루에 세 번 틀린 뒤 제정) — 같은 날 세 번 어긋났다. handoff 0049 "세 곳 모두 게스트 접근 가능"(한 곳은 아니었다), handoff 0050 "확인 절차가 전혀 없다"(커밋 `ffa6876`, 2026-08-06에 이미 있었다 — `service-desk/backlog.md` 028이 등록될 때부터 사실과 달랐다), 그리고 우선순위 판단의 "결제 수단이 안 열려 판매가 막혔다"(무통장 입금은 열려 있고 주문도 생성된다, handoff 0047 참고). 앞의 둘은 오래된 기록을 검증 없이 옮긴 것이고, 셋째는 같은 날 직접 본 사실들을 잘못 일반화한 것이다. **요청서는 "없으니 만들어달라"가 아니라 "있는지 확인하고 없으면 만들어달라"로 쓴다** — 0050에서 이 형식으로 쓴 (B) 덕분에 12개월 가드 부재가 드러났다. 그리고 같은 기준을 **판단 근거에도** 적용한다: 무엇을 먼저 할지가 하루치 작업 방향을 정하므로, 우선순위를 뒷받침하는 사실도 handoff 전제만큼 확인해야 한다.

## 관련 프로젝트: leedox-platform

`leedox-platform`(DEV, Rails 앱, 2026-08-03 이전 이름 chatdox-platform)이 `D:\RubyOnRails\leedox-platform`에 로컬 clone돼 있다(leedox-hq=HQ의 형제 폴더, 2026-08-03 이전 이름 chatdox-curriculum). 이 PC엔 클론이 두 벌 있다 — 편집용 Windows 마운트 클론과 테스트/서버 구동용 WSL 네이티브 클론(`~/dev/leedox-platform`, Windows 쪽은 CRLF 때문에 `bin/rails` 셔뱅이 깨짐).

**동기화**: 예전엔 git subtree로 HQ 전체를 `docs/curriculum/`에 받았으나(REQ 0022로 폐지), 지금은 `script/sync_curriculum.sh`가 HQ의 git 스냅샷에서 실제 쓰는 3개 폴더만 뽑아 `hq/chatdox/`, `hq/claudox/`, `hq/service-desk/`로 미러링한다(`hq/` 접두어로 "HQ가 제공, DEV 소유 아님"을 명시). **"동기화 없이 DEV가 HQ 폴더를 직접 바라보게 하면 안 되나?"는 2026-08-03에 두 번 나왔던 질문**(subtree 재도입, symlink) — 둘 다 막힌다: Railway는 DEV 자체 git 이력만 clone해서 빌드하므로 HQ나 Tommy 데스크탑을 아예 못 보고, symlink는 git이 대상 경로 "문자열"만 커밋할 뿐 링크된 폴더 내용은 추적하지 않아 HQ가 바뀌어도 DEV git엔 아무 diff도 안 잡히며 그 경로 자체도 데스크탑마다 다르다. 남은 개선 여지는 "커밋 단계 자체를 없애는 것"이 아니라 "스크립트 실행을 트리거하는 것"(예: HQ push 시 DEV에 sync PR을 자동으로 여는 GitHub Action)뿐이다.
...
프로덕션(`leedox.up.railway.app`)에 라이선스 보유 테스트 전용 계정이 있다 — 비밀번호 강화 규칙 이전에 만들어졌고 Tommy가 의도적으로 계속 그 상태로 둘 예정이라 보안 우려는 없다고 확인받았다. **로그인은 Tommy가 직접 한다** (2026-08-11 정정, handoff 0044 확인 과정에서 실증) — 예전엔 curl로 Devise 로그인(CSRF 토큰 추출 → 세션 쿠키)을 대신 수행했지만, 자격증명 대리입력은 하지 않는다(`claudox/98_hands_off_credentials.md`). 대신 **Tommy가 Chrome에서 로그인해둔 탭을 HQ가 이어받아** 확인한다 — 이미 인증된 세션에서 화면을 읽는 것이라 정책에 걸리지 않는다. 이 방식의 이점은 **한 세션에서 두 뷰어 상태를 다 볼 수 있다**는 것: 화면(DOM)은 라이선스 보유자 상태이고, 같은 페이지에서 `fetch(url, {credentials:'omit'})`로 요청하면 게스트가 받는 HTML을 그대로 받아 `DOMParser`로 비교할 수 있다(handoff 0044에서 부록 배지가 보유자=`열람 가능` 7건 / 게스트=`🔒 잠금` 7건으로 갈리는 것을 이 방법으로 한 번에 확인). 로그인 세션 안에서는 **읽기만 한다** — 결제·설정 변경·삭제 같은 조작은 하지 않고 필요하면 먼저 묻는다. 실제 이메일/비밀번호는 이 git 추적 파일에는 적지 않고 로컬 메모리(`reference_production_test_user.md`)에만 둔다 — credential은 아무리 저위험이어도 영구 git 이력에 남기지 않는다는 원칙. 상세 화면 확인이 필요할 때 매번 물어보지 않고 바로 이 계정을 쓴다.
